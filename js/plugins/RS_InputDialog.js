...
(function () {
  ...
  RS.InputDialog.setRect = function () {
    ...
    if (textBox) {
      textBox.style.fontSize = (Utils.isMobileDevice()) ? '1rem' : (3.2 * Graphics._realScale) + "em";
  ...
  TextBox.prototype.onFocus = function (e) {
    ...
    if (text && Utils.isMobileDevice()) {
      text.style.bottom = RS.InputDialog.getScreenHeight(Graphics.boxHeight * Graphics._realScale / 2) + 'px';
    }
  };
  ...
  TextBox.prototype.setPosition = function (x, y) {
    ...
    if (field) {
      ...
      if (x > (Graphics.boxWidth - RS.InputDialog.Params.textBoxWidth) * Graphics._realScale) {
        x = (Graphics.boxWidth - RS.InputDialog.Params.textBoxWidth) * Graphics._realScale;
      }
      if (y < 0) {
        y = 0;
      }
      if (y > (Graphics.boxHeight - RS.InputDialog.Params.textBoxHeight) * Graphics._realScale) {
        y = (Graphics.boxHeight - RS.InputDialog.Params.textBoxHeight) * Graphics._realScale;
      }
  ...
  TextBox.prototype.onResize = function () {
    ...
    if (field && textBox) {
      ...
      if (RS.InputDialog.Params.isCenterAlignment) {
        var px = ((Graphics.boxWidth / 2) - (RS.InputDialog.Params.textBoxWidth / 2)) * Graphics._realScale;
        var py = ((Graphics.boxHeight / 2) - (RS.InputDialog.Params.textBoxHeight / 2)) * Graphics._realScale;
