...
Scene_Input.prototype.onThatsJustFine = function () {
    ...
    $gameVariables.setValue(Darkkitten.Param.varId, str);
...
Window_TextEdit.prototype.initialize = function () {
    ...
    this._text = ""; //Change: removed default text completely
...
Window_TextInput.prototype.cursorPagedown = function () {
    if (Input.anyTriggered("a-z")) return;
    ...
    if (this.isOpen() && this.active) {
        if (Imported.Quasi_Input || Imported.QInput) {
            if (Input.isTriggered('#enter')) {
                ...
                //else { this.processJump(); } //Change: no jumping to the enter button, we just start there
            }
            else if (Input.anyPressed("a-z0-9")) {
                if (!Input.isPressed('#enter') && (!this._skipSpace || Input._lastPressed !== ' ')) {
                    console.log("Adding", Input._lastPressed);
                    this._editWindow.add(Input._lastPressed);
                    this._skipSpace = false;
                } //else if (Input._lastPressed === ' ') this._skipSpace = false;
